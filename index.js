...
function createLogEntry(entry) {
    document.getElementById('log').innerText += '<br>' + entry;
}
...
function createModel() {
    ...
    createLogEntry('Start compilimng ...');
    model.compile({
        optimizer: tf.train.sgd(0.15),
        loss: 'categoricalCrossentropy'
    });
    createLogEntry('Complited');
}
...
async function load() {
    ...
    data = new MnistData();
    await data.load();
    createLogEntry('Data loaded succeefully');
}
...
async function train() {
    createLogEntry('Starting training ...');
...
async function main() {
    ...
    await load();
    await train();
    document.getElementById('selectTetstDataButton').disabled = false;
    document.getElementById('seletedTestDataButton').innerText = "Randomly Select Test Data And Predict";
}
